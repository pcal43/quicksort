...
public class QuicksortInitializer implements ModInitializer {
    ...
    private void initialize() throws IOException {
        ...
        //
        // Always write out the default config to the .disabled so the can rename/edit
        // it if they want.
        //
        ...
        if (!configFilePath.toFile().exists()) {
            ...
            try (final InputStream in = QuicksortInitializer.class.getClassLoader()
                    .getResourceAsStream(DEFAULT_CONFIG_RESOURCE_NAME)) {
                if (in == null)
        ...
        try (final InputStream in = new FileInputStream(configFilePath.toFile())) {
            // parse the config file using the first chest config from our embedded defaults
            // as the default
